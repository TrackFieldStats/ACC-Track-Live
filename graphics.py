...
def chart_current_standings(team_scores: list[TeamScore], gender: Gender,
                              meet_name: str = "SEC Championships") -> bytes:
...
def chart_projected_scores(team_scores: list[TeamScore], gender: Gender,
                             meet_name: str = "SEC Championships") -> bytes:
    """
    Grouped bar chart showing actual / seed projection / monte carlo expected
    ...
    expected = [ts.monte_carlo_expected for ts in top]
    ...
    b3 = ax.bar(x + width, expected, width, label="MC Expected",
                color=ACCENT_GREEN, alpha=0.9, zorder=3)
...
def chart_leverage_index(leverage_data: list[dict], gender: Gender,
    ...
    narratives = [d.get("narrative", "") for d in top_rev]

    # Color by narrative type
    narrative_colors = {
        "🔒 Clincher": "#f85149",   # red — decisive
        "🔥 Contested": "#f0c040",  # gold — competitive
        "📈 Swing":     "#3fb950",  # green — comeback potential
    }
    bar_colors = [narrative_colors.get(n, "#8b949e") for n in narratives]
    ...
    legend_elements = [
        Patch(facecolor="#f85149", label="🔒 Clincher — leader could pull away"),
        Patch(facecolor="#f0c040", label="🔥 Contested — multiple teams fighting"),
        Patch(facecolor="#3fb950", label="📈 Swing — comeback opportunity"),
    ]
...
def chart_team_scenarios(scenario: dict, meet_name: str = "SEC Championships") -> bytes:
    """
...
def generate_social_bundle(
    ...
    men_analysis: dict,
    output_dir: str = ".",
    meet_name: str = "SEC Championships"
) -> list[str]:
    """

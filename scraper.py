...
def parse_index(meet_url: str) -> list[dict]:
    ...
    # Deduplicate: if two index rows point to the same compiled URL, keep only
    # the one with the fuller/cleaner event name (no "Section X" suffix).
    # This handles ACC-style sectioned distance events where both "Event (Section 1)"
    # and the combined "Event" rows link to the same compiled results page.
    seen_compiled: dict[str, dict] = {}
    for ev in events:
        url = ev["compiled_url"]
        if url not in seen_compiled:
            seen_compiled[url] = ev
        else:
            existing = seen_compiled[url]
            # Prefer the name WITHOUT a "Section" qualifier
            if "section" in existing["event_name"].lower() and "section" not in ev["event_name"].lower():
                seen_compiled[url] = ev
    events = list(seen_compiled.values())
...
def _split_athlete_team(raw: str) -> tuple[str, str]:
    ...
    for team in ALL_CAPS_TEAMS:
        if raw.upper().endswith(team.upper()):
            ...
            if name_part:
                return name_part, _normalize_team(team)
    ...
    if match:
        ...
        return name_part, _normalize_team(team_part)
...
def _parse_result_page(soup: BeautifulSoup, is_start_list: bool = False) -> tuple[list[Athlete], EventStatus]:
    ...
    for table in tables:
        ...
        for cells in data_rows:
            ...
            if is_relay_table:
                # Two relay HTML formats exist:
                # Format A (DMR): <b> S. CAROLINA</b><br><small>South Carolina</small>
                #   → abbrev in <b>, full name in <small>
                # Format B (4x400): <b><a>South Carolina</a></b><br><small>SCAR A</small>
                #   → full name in <a>, abbrev in <small>
                # Fix: pick whichever of <a> text vs <small> text is longer = full name
                a_text = bold_a.get_text(strip=True) if bold_a else ""
                small_text = small.get_text(strip=True) if small else ""
                # Strip relay letter suffixes like " A", " B" and year tags from both
                a_text = re.sub(r'\s*\[(?:JR|SR|FR|SO|\d+)\]\s*$', '', a_text, flags=re.IGNORECASE).strip()
                a_text = re.sub(r'\s+[A-Z]$', '', a_text).strip()
                small_text = re.sub(r'\s*\[(?:JR|SR|FR|SO|\d+)\]\s*$', '', small_text, flags=re.IGNORECASE).strip()
                small_text = re.sub(r'\s+[A-Z]$', '', small_text).strip()
                # Pick the longer one — full team name is always longer than abbreviation
                team = a_text if len(a_text) >= len(small_text) else small_text
                team = _normalize_team(team)
                if not team:
                    team = raw_athlete.title()
                if not team:
                    continue
                name = f"{team} Relay"
            elif small:
                # Individual athlete cell formats:
                #   With link:    <b><a>Madison CHILDRESS</a></b><sup> 12</sup><br><small>South Carolina [JR]</small>
                #   Without link: <b>Elsa SPOOR</b><sup> 94</sup><br><small>Virginia [SO]</small>
                # Name from <a> if present, else from <b> tag directly (avoids <sup> bib bleed)
                if bold_a:
                    name = bold_a.get_text(strip=True)
                else:
                    b_tag = cell.find("b")
                    name = b_tag.get_text(strip=True) if b_tag else raw_athlete
                team = _normalize_team(re.sub(r'\s*\[(?:JR|SR|FR|SO|\d+)\]\s*$', '', small.get_text(strip=True), flags=re.IGNORECASE).strip())
            else:
...
def _parse_scores_page(soup: BeautifulSoup, event_name: str, gender: Gender) -> CombinedEventResult:
    ...
    Completion detection: FlashResults marks the modal title with [SCORED] when
    all sub-events are complete, and [STANDINGS] when still in progress.
    We use this as the authoritative completion signal.
    """
    ...
    # --- Completion check via [SCORED] / [STANDINGS] tag in modal title ---
    # e.g. "Women Pentathlon: Projected Points   [SCORED]"  → complete
    #      "Men Heptathlon: Projected Points   [STANDINGS]" → in progress
    is_scored = False
    modal_title = soup.find(id="projectedPointsModal")
    if modal_title:
        title_text = modal_title.get_text(strip=True)
        is_scored = "[SCORED]" in title_text.upper()
    else:
        # Fallback: look anywhere in the page for [SCORED]
        page_text = soup.get_text()
        is_scored = "[SCORED]" in page_text.upper()
    ...
    for table in tables:
        ...
        for row in rows[1:]:
            ...
            # Name cell: extract from <a> tag to avoid projected score data bleeding in
            # Structure: <b><a href="#data">Janie FORD</a></b>  Vanderbilt [FR]
            #            <i>projectedPB4337 | diff 36</i>
            ...
            if a_tag:
                ...
                lines = [
                    ln.strip() for ln in name_cell.get_text(separator="\n").splitlines()
                    if ln.strip() and "|" not in ln and "projected" not in ln.lower()
                ]
            ...
            # Team extraction
            # SEC format: separate "Team"/"School" column (team_idx is set)
            # ACC format: team name is plain text in the same cell as the athlete name,
            #   on the line immediately after the </b></a> tag, before any <span> projected data
            #   e.g.: <b><a>Seren RODGERS</a></b><br>Wake Forest [SO]</br><span>projected...</span>
            if team_idx is not None:
                team_cell = cells[team_idx]
                team_lines = [
                    ln.strip() for ln in team_cell.get_text(separator="\n").splitlines()
                    if ln.strip() and "|" not in ln and "projected" not in ln.lower()
                    and not any(c.isdigit() for c in ln)
                ]
                team = team_lines[0] if team_lines else ct(team_idx)
                name = raw_name
            else:
                # ACC format: parse team from the athlete cell itself
                # Strategy: get all text lines from the cell, skip the name line,
                # skip projected/diff lines, take the first remaining line as team
                name = raw_name
                team = ""
                cell_lines = [
                    ln.strip()
                    for ln in name_cell.get_text(separator="\n").splitlines()
                    if ln.strip()
                ]
                for ln in cell_lines:
                    if ln == raw_name:
                        continue
                    if "|" in ln or "projected" in ln.lower():
                        continue
                    # Strip year tags like [FR], [SO], [JR], [SR]
                    ln_clean = re.sub(r'\s*\[(?:JR|SR|FR|SO|\d+)\]\s*$', '', ln, flags=re.IGNORECASE).strip()
                    if ln_clean and not ln_clean.isdigit() and len(ln_clean) > 1:
                        team = _normalize_team(ln_clean)
                        break
        ...
        if athletes:
            ...
            # Only mark FINAL if the page itself signals completion via [SCORED]
            # A multi-event with interim standings has places assigned but is NOT done
            result.status = EventStatus.FINAL if is_scored else EventStatus.IN_PROGRESS
            break
...
def scrape_meet(meet_url: str, prev_state: "MeetState" = None) -> MeetState:
    ...
    If prev_state is provided, any event already marked FINAL is reused
    from cache without re-fetching — significantly speeds up refreshes.
    """
    logger.info(f"Scraping meet: {meet_url}")

    # Build cache of completed regular events keyed by compiled_url
    # Combined events (Pent/Hep) are NOT cached — complex HTML, only 1-2 pages
    final_cache: dict[str, MeetEvent] = {}
    if prev_state:
        for ev in prev_state.events:
            if ev.status == EventStatus.FINAL and ev.compiled_url:
                final_cache[ev.compiled_url] = ev
    ...
    for raw in raw_events:
        ...
        # Skip if already marked FINAL in previous state
        if raw["compiled_url"] in final_cache:
            logger.debug(f"Cache hit: {raw['compiled_url']}")
            state.events.append(final_cache[raw["compiled_url"]])
            continue
        ...
        # Also backfill seed marks for In Progress events — athletes currently racing
        # have no mark on the compiled page, but the start list has their SB.
        ...
        elif status == EventStatus.IN_PROGRESS and raw["start_url"]:
            # Backfill seed marks only if athletes are missing them
            no_seeds = [a for a in athletes if not a.seed_mark]
            if no_seeds:
                time.sleep(REQUEST_DELAY)
                start_soup = _get(raw["start_url"])
                start_athletes, _ = _parse_result_page(start_soup, is_start_list=True)
                seed_lookup = {a.name: a.seed_mark for a in start_athletes if a.seed_mark}
                for a in athletes:
                    if not a.seed_mark and a.name in seed_lookup:
                        a.seed_mark = seed_lookup[a.name]
